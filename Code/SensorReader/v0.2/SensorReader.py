...
class AdcRtd_Client(object):
    """
    Customized class to combine ADC and RTD readings
...
class BaseReader(object):
    ...
    def __init__(self, sensor_name='sensor', sensor_location='location', dropbox_keys=None, 
                 delimiter=',', output_file_ext='.csv', output_file_folder='Outputs'):
        """
        ...
        Parameters:
            ...
            Argument 5: output_file_ext (string, optional):
                The file extension of the output file.
                Defaults to ".csv" (comma separated values).
                
            Argument 6: output_file_folder (string, optional):
        ...
        if not isdir(output_file_folder): makedirs(output_file_folder)
    ...
    def start_dropbox_upload(self, interval):
        ...
        while True:
            ...
            upload_thread = Thread(target=self.upload_to_dropbox)  # initiate upload on a separate thread
            upload_thread.start()
...
class ModbusSerialReader(BaseReader):
    ...
    def __init__(self, port, parity, baudrate, timeout=0.05, sensor_name='sensor', sensor_location='location', 
                 dropbox_keys=None, delimiter=',', output_file_ext='.csv', output_file_folder='Outputs'):
        """
        ...
        Parameters:
            ...
            Argument 4: timeout (float)
                The amount of time given for the client to read per request.
                
            Argument 5, 6, 7, 8, 9, 10: See BaseReader
...
class AdcI2CReader(BaseReader):
    ...
    def __init__(self, bit, pga, sps, sensor_name='sensor', sensor_location='location', dropbox_keys=None, 
                 delimiter=',', output_file_ext='.csv', output_file_folder='Outputs'):
        """
        ...
        Parameters:
            ...
            Argument 4, 5, 6, 7, 8, 9: See BaseReader
...
class RtdSPIReader(BaseReader):
    ...
    def __init__(self, bus, channel, sensor_name='sensor', sensor_location='location', dropbox_keys=None, 
                 delimiter=',', output_file_ext='.csv', output_file_folder='Outputs'):
        """
        ...
        Parameters:
            ...
            Argument 3, 4, 5, 6, 7, 8: See BaseReader
...
class IrridianceReader(BaseReader):
    """"
    A class to manage the connection to a irridiance sensor.
    ...
    Attributes:
        ...
        client (IrridianceClient):
            Object of the Irridiance client.
    
    """
    # Defining the dictionary keys of a request
    KEYS = {
        'NAME': 'name',
        'TYPE': 'type',
        'CHANNEL': 'channel',
    ...
    def __init__(self, adc_bit, adc_pga, adc_sps, rtd_bus, rtd_channel, sensor_name='sensor', 
                 sensor_location='location', dropbox_keys=None, delimiter=',', 
                 output_file_ext='.csv', output_file_folder='Outputs'):
        """
        Constructor method of the IrridianceReader class.
        The Irridiance client is initialized.
        ...
        Parameters:
            ...
            Argument 6, 7, 8, 9, 10, 11: See BaseReader

        """
        super(IrridianceReader, self).__init__(sensor_name=sensor_name,
                                               sensor_location=sensor_location,
                                               dropbox_keys=dropbox_keys,
                                               delimiter=delimiter,
                                               output_file_ext=output_file_ext,
    ...
    def execute_request(self, request):
        ...
        if request[self.KEYS['TYPE']] == 'I2C':
            return self.client.read_adc(request[self.KEYS['CHANNEL']], self.pga, self.sps)
        if request[self.KEYS['TYPE']] == 'SPI':
            return self.client.read_rtd()
    ...
    def read_all_requests(self, requests, print_output):
        ...
        with open(self.output_filename, 'ab+') as output_file:  # ab+: append-only in binary, backward readable
            ...
            print 'Reading data on ' + str(datetime.now()) + '...'
            for request in requests:
                if not request[self.KEYS['TYPE']] == 'derived':
                    result = self.execute_request(request)
                if request[self.KEYS['NAME']] == 'mini-volts':
                    mini_voltage = result
                if request[self.KEYS['NAME']] == 'temperature':
                    temperature = result
                if request[self.KEYS['NAME']] == 'irridiance':
                    #result = mini_voltage / 54.3 * 1000 * (1 - 0.0005 * (temperature - 25))
                    result = mini_voltage / 54.57 * 1000 / (1 + 0.0005 * (temperature - 25))
                if print_output: print result,
                output.append(str(result))
            end_read_time = time()
